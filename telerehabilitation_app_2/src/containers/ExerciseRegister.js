...
const ExerciseRegisterScreen = ({route, navigation}) => {
  const cameraRef = useRef(null);

  const videoRecord = async () => {
    if (cameraRef && cameraRef.current) {
      cameraRef.current.open({maxLength: 600}, data => {
        console.log('captured data', data);
        CameraRoll.save(data.uri);
        console.log('saved?');
        console.log(data.uri);
        navigation.navigate('upload-exercise', {videoUri: data.uri});
      });
    }
  };
      ...
      {!permissionsLoading && permissionsGranted && device != null ? (
        <>
          ...
          {/* <View>
            ...
            <Button onPress={() => videoRecord()} title="Grabar" />
          </View> */}
          <View style={styles.actions}>
            <VideoRecorder ref={cameraRef} />
            <TouchableOpacity
